...
name: mui
description: >-
  Material-UI component patterns (sx prop, theme, responsive layout, hooks).
  Vendored from softaworks/agent-toolkit skills/mui. Use when editing StoreDesk
  Electron UI with @mui/material, sx styling, ThemeProvider, createTheme, Grid,
  Stack, Box, Dialog, TextField, or MUI icons.
---

# MUI Patterns

> Source: [softaworks/agent-toolkit skills/mui](https://github.com/softaworks/agent-toolkit/tree/main/skills/mui)

## StoreDesk adaptation

- Canonical Cursor path only: **`.cursor/skills/mui/`** (no Claude / `.claude` copies).
- Desktop app uses **`@mui/material` ^6.4** — prefer MUI MCP docs for the installed major when APIs differ from v7 notes below.
- Product layout / no-subtitle rules: **`storedesk-ui`** (`.cursor/skills/storedesk-ui/SKILL.md`).
- Theme of record: `store-desk-electron/src/theme/`. Brand: navy primary + green secondary.
- Prefer flat outlined surfaces (`variant="outlined"`) over glow/shadow “dashboard kits.”
- Prefer package imports (`@mui/material`, `@mui/icons-material`) — no deep imports.
- For official prop/API questions: MUI MCP (`useMuiDocs` → `fetchDocs`).
- Codex: thin mirror under `.codex/skills/mui/` points here.

## Purpose

Material-UI patterns for component usage, styling with sx prop, theme integration, and responsive design.

**MUI v7 notes** (ahead of StoreDesk's current v6; use when upgrading or when MCP confirms):
...
- StoreDesk Electron page/component UI work (with `storedesk-ui`)
