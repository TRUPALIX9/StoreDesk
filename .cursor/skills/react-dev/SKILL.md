...
version: 1.0.0
description: >-
  Type-safe React + TypeScript patterns (props, events, hooks, generics, React Router).
  Vendored from softaworks/agent-toolkit skills/react-dev. Use when editing StoreDesk
  Electron React/TSX components, hooks, forms, or React Router typing.
---

# React TypeScript

> Source: [softaworks/agent-toolkit skills/react-dev](https://github.com/softaworks/agent-toolkit/tree/main/skills/react-dev)

## StoreDesk adaptation

- Desktop app: **React 19** + **TypeScript** + **React Router v7** (HashRouter) + **TanStack Query** + **MUI** in `store-desk-electron/`.
- Prefer client-component patterns — Electron renderer is not Next.js Server Components. Skip Server Actions / RSC sections unless building a separate web app.
- Routing reference: `references/react-router.md` (not TanStack Router unless explicitly chosen).
- With UI work also load: `.cursor/skills/mui/SKILL.md` + `.cursor/skills/storedesk-ui/SKILL.md`.
- Keep business logic in services/`api/`; type components and hooks in pages/modules.
...
- StoreDesk Electron React/TypeScript work

NOT for: non-React TypeScript, vanilla JS React, Flutter/Dart (StoreDesk Buddy)
